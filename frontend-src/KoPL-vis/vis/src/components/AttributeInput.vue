...
<script>
import bus from './bus';
import { AllAttrsInSmallKB } from './smallAttr';
import { AllAttrsInLargeKB } from './largeAttr';
    ...
    handleSearch(value) {
      if (value) {
        if (this.whichKB == 'small') {
          ...
          else this.filterList = AllAttrsInSmallKB.filter(a => { return a.indexOf(value) != -1; });
        } 
          ...
          else this.filterList = AllAttrsInLargeKB.filter(a => { return a.indexOf(value) != -1; });
        }
      } else {
        this.filterList = [];
      }
